# get-interfaces with topology by name don't use Defined by Routes

**Collection:** Web API (version 2.1) > 64 Network Interface
**Method:** `POST`
**URL:** `{{server}}/v2.1/get-interfaces`

## Description

Get interfaces with topology from target gateway or cluster by name, don't use Defined by Routes topology

## Request Headers

| Header | Value |
|--------|-------|
| Content-Type |  application/json |
| X-chkp-sid |  {{session}} |

## Request Body

**Mode:** `raw`

```json
{
  "target-name": "gw123",
  "with-topology": true,
  "use-defined-by-routes": false
}
```

## Example Responses

### Example 1: get-interfaces with topology by name don't use Defined by Routes
**Status:** `200 OK`

**Body:**
```javascript
{
  "tasks": [
    {
      "task-name": "get-interfaces",
      "task-id": "01234567-89ab-cdef-9e1f-0e1e68312345",
      "status": "succeeded",
      "progress-percentage": 100,
      "suppressed": false,
      "task-details": [
        {
          "interfaces": [
            "eth0",
            "eth1"
          ]
        }
      ]
    }
  ]
}
```
