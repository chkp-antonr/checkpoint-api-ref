...
```json
{
  "command": "set-smart-task",
  "description": "Edit existing object using object name or uid. This command is available only in a Security Management environment or in Multi-Domain environment when logged into local domain.",
  "request": {
    "url": "POST https://<mgmt-server>:<port>/web_api/set-smart-task",
    "headers": [
      {
        "name": "Content-Type",
  ...
  "metadata": {
    "version": "2.0.1",
    "extracted_at": "2025-10-05T21:36:20.444080",
    "source_file": "set-smart-task.html"
  }
}
```