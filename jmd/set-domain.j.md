...
{
  "command": "set-domain",
  "description": "Edit domain object using domain name or UID. When the list of domain servers is edited, the command is handled asynchronously. A list of task identifiers is returned to a user. In this case, the changes to the domain object are done in a public session and so should not be published. If the domain is changed in other parameters than the domain servers, i.e.: comments, color or tags, such changes are done in the user's private session and therefore should be published. In this case, the returned command output is similar to the one of 'show-domain'.",
  "request": {
    "url": "POST https://<mgmt-server>:<port>/web_api/set-domain",
    "headers": [
      {
        "name": "Content-Type",
    ...
    "body": [
      ...
      {
        "name": "ipv6-address",
        "description": "IPv6 address. Can only be set one time.",
        "type": "string",
        "required": false
      },
      {
        "name": "skip-start-domain-server",
  ...
  "response": {
    "success": [
      ...
      {
        "name": "global-domain-assignments",
        "description": "",
        "type": "array",
        "required": false,
        "valid_values": [
  ...
  "examples": {
    "set-domain": {
      "description": "",
      "request": "POST {{server}}/set-domain\nContent-Type: application/json\nX-chkp-sid: {{session}}\n\n{\n  \"name\" : \"domain1\",\n  \"comments\" : \"This is domain1 comment\"\n}",
      "response": ""
    },
    "set-domain add log server": {
      "description": "Edit an existing domain, add a log server",
  ...
  "metadata": {
    "version": "2.0.1",
    "extracted_at": "2025-10-05T21:36:16.652730",
    "source_file": "set-domain.html"
  }
}
```