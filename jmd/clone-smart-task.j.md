...
```json
{
  "command": "clone-smart-task",
  "description": "clone existing object using object name or uid. This command is available only in a Security Management environment or in Multi-Domain environment when logged into local domain.",
  "request": {
    "url": "POST https://<mgmt-server>:<port>/web_api/clone-smart-task",
    "headers": [
      {
        "name": "Content-Type",
  ...
  "metadata": {
    "version": "2.0.1",
    "extracted_at": "2025-10-05T21:36:13.753745",
    "source_file": "clone-smart-task.html"
  }
}
```